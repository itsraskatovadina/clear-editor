...
class ANode(object):
	...
	def info(self):
		return "".join([self.nid, "; par: ", str(self.parent),
			"; lvl: ", str(self.lvl), "; subs: ", str(self.children)])
	...
	def set_parent(self, node):
		if isinstance(node, __class__):
			...
			node_type = type(node)
			Tools.output(f"Invalid argument type node in ANode.set_parent(), {node_type}",
				outtype = "error")
	...
	def add_child(self, node):
		if isinstance(node, __class__):
			...
			node_type = type(node)
			Tools.output(f"Invalid argument type node in ANode.add_child(), {node_type}",
				outtype = "error")
	...
	def add_children(self, subnodes):
		if isinstance(subnodes, list):
			...
			Tools.output(f"Invalid argument type node in ANode.add_children()",
				outtype = "error")
...
class Iclear(object):
	...
	def num_level(level):
		...
		num_level = Iclear.levels_index.get(level) 
		if num_level == None:
			Tools.output(f"Invalid argument type node in ANode.num_level()",
				outtype = "error")
			return False
		return num_level
	...
	def next_level(level):	
		...
		num_level =  Iclear.num_level(level)	#  Critical Error, if num_level == None (not finded)
	...
	def fill_sites(self):
		...
		for ul in uls:
			...
			for li in lis:
				...
				top.add_child(man)
			if top.size() == 0:
				Tools.output(f"No detected mans in ISite.fill_sites {top.nid}", outtype = "info")
		if self.size() == 0:
			Tools.output(f"No detected tops in ISite.fill_sites {self.nid}", outtype = "info")
		return True
	...
	def fill(self):
		...
		for ul in uls:
			...
			for li in lis:
				...
				cat.add_child(page)
			if cat.size() == 0:
				Tools.output(f"No detected pages in IMan.fill {cat.nid}", outtype = "info")
		if self.size() == 0:
...
class NodesNav(Iclear):
	...
	def set_from_url(self, url):
		...
		if len(url_parts) > 3: sitenid = url_parts[3]
		if len(url_parts) > 5: topnid = url_parts[5]
		if len(url_parts) > 6: mannid = url_parts[6]
		if len(url_parts) > 8: catnid = url_parts[8]
		if len(url_parts) > 9: pagenid = url_parts[9].split(sep=".")[0]
			
		nodedict = {'site': sitenid, 'top': topnid, 'man': mannid, "cat": catnid, "page": pagenid}
...
<ul id = "hiddenmenu">\n'
		...
		for cat in man.children:
			asidetext = asidetext + f'\t<li id = "menu_{cat.nid}"><details><summary>{cat.name}</summary><ul>\n'
...
<script src="../../../../../../app/scripts/script.js"></script>'
		partheadname = os.path.join(man.full_path(), man.host().phpincdir, "parthead.php")
		#print(partheadname, "\n", partheadtext)
...
'		
		...
		for top in site.children:
			...
			for man in top.children:
				...
				for cat in man.children:
					for page in cat.children: pagecount+=1
...
<body onload="start()">\n\
	...
	def diff_map_by_dir(man):
		...
		*в папке справочника (manual) должны быть только папки каталогов из карты)"""
		...
		if (not error_found):
			Tools.output("Карта справочника '"+ man.fname +"' соответствует файловой структуре.")
